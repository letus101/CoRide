...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
        ...
        move_uploaded_file($image_tmp, 'media/avatar/' . $image_new_name);
            ...
            <div class="p-4">
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="fname" name="fname" value="<?= $user['FNAME'] ?>" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-blue-500" required>
                    </div>
                    <div class="mb-4">
                        <label for="lname" class="block text-sm font-semibold mb-2">Last Name:</label>
                        <input type="text" id="lname" name="lname" value="<?= $user['LNAME'] ?>" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-blue-500" required>
                    </div>
                    <div class="mb-4">
                        <label for="email" class="block text-sm font-semibold mb-2">Email:</label>
                        <input type="email" id="email" name="email" value="<?= $user['EMAIL'] ?>" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-blue-500" required>
                    </div>
                    <div class="mb-4">
                        <label for="phone" class="block text-sm font-semibold mb-2">Phone:</label>
                        <input type="text" id="phone" name="phone" value="<?= $user['PHONE'] ?>" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-blue-500" required>
                    </div>
